...
*  [为什么B+树要这样设计？](#2.16)
*  [mysql常见的优化](#2.17)
*  [redolog/undolog/binlog 的区别？binlog 的几种格式？说下两阶段提交？](#2.18)
*  [执行 insert 语句的过程是什么](#2.19)
*  [事务隔离级别和不同级别会出现的问题，innodb 默认哪个级别？MVCC 怎么实现的？快照读和当前读有啥区别？幻读的问题怎么解决？](#2.20)
*  [主从同步流程（异步同步）](#2.21)
*  [主从同步，数据库主库和从库不一致，常见有这么几种优化方案](#2.22)
*  [InnoDB 内存结构包含四大核心组件](#2.23)
*  [explain 的 type 字段有哪些](#2.24)
*  [MySQL中update语句的执行流程](#2.25)
*  [死锁什么时候会出现？应用层应该怎么做避免死锁？mysql 是怎么处理死锁的呢？](#2.26)
*  [MySQL 遇到过死锁问题吗，你是如何解决的？](#2.27)
*  [常用的分库分表中间件](#2.28)
*  [分库分表可能遇到的问题](#2.29)
*  [InnoDB 四种事务隔离级别](#2.30)
*  [select for update 有什么含义，会锁表还是锁行还是其他？](#2.31)
*  [int (3) 和 int (11) 有区别吗？](#2.32)
*  [sql 的执行流程](#2.33)
*  [MySQL 索引使用有哪些注意事项呢？](#2.34)
*  [索引哪些情况会失效](#2.35)
*  [MySQL 锁的分类，作用，你在实际工作的中使用场景](#2.36)
</details>

<details>
<summary>Redis</summary>

*  [sds 的结构是什么？为什么要存长度？跟 c 里的字符串有什么区别？](#3.1)
*  [缓存如何保证一致性](#3.2)
*  [用过 redis 哪些数据结构，使用场景是什么](#3.3)
*  [redis 的 connect 和 pconnect 的区别，pconnect 有什么问题](#3.4)
*  [redis 如何实现分布式锁，有什么问题](#3.5)
*  [Redlock 具体如何使用](#3.6)
*  [为什么Redis一定要用跳表来实现有序集合](#3.8)
*  [redis主从同步的原理](#3.9)
*  [Redis哨兵、复制、集群的设计原理与区别](#3.10)
*  [redis cluster 用的什么协议同步数据](#3.11)
*  [redis哨兵的选举用的什么协议](#3.12)
*  [hash 怎么实现的？怎么解决 hash 冲突？除了 hashTable 还有别的吗？](#3.13)
*  [zset 怎么实现的？跳表是怎么插入的？为什么选择跳表不用其他平衡二叉树？除了跳表还有别的吗？](#3.14)
*  [为什么 redis 用跳表？](#3.15)
*  [rehash 过程？会主动 rehash 吗？](#3.16)
*  [用 redis 可以实现队列吗？有什么优点和缺点？](#3.17)
*  [用 redis 怎么实现一个延时队列？](#3.18)
*  [rdb 和 aof 过程？rdb 为什么可以用创建子进程的方式进行？（这里考察一个 cow）这两种持久化方式会丢数据吗？](#3.19)
*  [数据过期和淘汰策略](#3.20)
*  [Redis的内存淘汰策略](#3.21)
*  [Redis集群方案应该怎么做？都有哪些方案？](#3.22)
*  [Redis集群方案什么情况下会导致整个集群不可用？](#3.23)
*  [说说Redis哈希槽的概念](#3.24)
*  [Redis集群的主从复制模型是怎样的](#3.25)
*  [Redis集群会有写操作丢失吗？为什么？](#3.26)
*  [怎么理解Redis事务？](#3.27)
*  [Redis事务相关的命令有哪几个？](#3.28)
*  [Redis持久化数据和缓存怎么做扩容？](#3.29)
*  [缓存雪崩 击穿 穿透](#3.30)
*  [redis 为什么快？（主要考察一个 IO 多路复用和单线程不加锁）](#3.31)
*  [一致性哈希是什么？节点较少时数据分布不均匀怎么办？](#3.32)
*  [简单说下几种 key 的淘汰策略，redis 里的 lru 算法，什么时候会触发？实现细节是什么？怎么保证淘汰合理的 key？](#3.33)
*  [lua 脚本的作用是什么？](#3.34)
*  [缓存击穿 / 穿透 / 雪崩的处理策略](#3.35)
</details>
...
*  [描述XSS注入原理，以及如何防止？](#5.1)
*  [描述HTML 5中新增的 EventSource 的功能和应用场景？](#5.2)
*  [ES 6中的`Promise`对象是做什么的？](#5.3)
*  [解释ES 6中`async、await`的使用场景？](#5.4)
*  [ES 6中 遍历器`Iterator`怎么写，其作用是什么？](#5.5)
*  [写出下面代码执行后输出的内容](#5.6)
*  [vue 和 angularJS 中检测脏数据的原理有什么区别？](#5.7)
*  [vue中，vuex的主要作用是什么？](#5.8)
*  [vue中 data 和computed 有什么区别？](#5.9)
</details>

<details>
<summary>通信协议</summary>

*  [详细描述 HTTPS（SSL）工作原理？](#6.1)
*  [TCP三次握手](#6.2)
*  [TCP 四次挥手](#6.3)
*  [服务器使用PHP时，客户端的IP能伪造吗？如果能，列出伪造方法；如果不能，说明原因？](#6.4)
*  [描述域名劫持的各种方法，为什么HTTPS不能被劫持？](#6.5)
*  [描述HTTP协议是什么，以及`HTTP 2` 和 `HTTP 1.1` 有什么区别？](#6.6)
*  [详细描述IP协议、TCP协议，以及UDP协议与它们的区别。](#6.7)
*  [TCP协议中，最大传输单元MTU一般最大是多少，在TCP协议中，如果一个数据被分割成多个包，这些包结构中什么字段会被标记相同。](#6.8)
*  [UDP分包和TCP分包会有哪些区别？](#6.9)
*  [HTTP协议中 `Transfer-Encoding: Chunked` 适用于哪些应用场景，这个与使用 `Content-Length: xxx` 在收到的报文包上有哪些区别？](#6.10)
</details>

<details>
<summary>kafka</summary>

*  [kafka 的架构，大致储存结构](#7.1)
*  [如果消费者数超过分区数会怎么样?](#7.2)
*  [怎么保证数据的可靠投递？](#7.3)
*  [消费者的 offset 存在哪里？](#7.4)
*  [如何通过 offset 定位消息？](#7.5)
</details>

<details>
<summary>其他</summary>

*  [描述`epoll`和`poll`、`select`的区别，为什么`epoll`会具备性能优势？](#8.1)
*  [描述下惊群的原因？有什么有效的方法可以避免惊群？](#8.2)
*  [什么是Hash一致性，这个方法主要运用在什么场景？](#8.3)
*  [设计一个多重缓存的拓扑结构](#8.4)
*  [IO 多路复用是什么？有哪些 api？](#8.5)
*  [select 和 epoll 的区别？水平触发和边缘触发的区别是啥？使用的时候需要注意什么？](#8.6)
*  [epoll 储存描述符的数据结构是什么？](#8.7)
*  [select 有描述符限制吗？是多少？](#8.8)
*  [进程 / 线程 / 协程区别？go 和 swoole 的协程实现有啥区别？](#8.9)
*  [swoole 协程的原理？](#8.10)
*  [描述OAuth2的工作原理？](#8.11)
*  [列出几个中文分词工具？](#8.12)
*  [git 放弃未提交的文件有哪些方法？](#8.13)
*  [git删除远程分支、Tag有什么方法？](#8.14)
*  [git覆盖远程仓库有什么办法？](#8.15)
*  [CentOS 下安装php扩展有哪些方法？](#8.16)
*  [布隆过滤器，什么时候用？优点是什么？](#8.17)
*  [硬盘如何储存数据的？](#8.18)
*  [设计秒杀系统，需要支持 100W 以上 QPS](#8.19)
*  [设计微博首页，需要拉取所有关注用户的最近 20 条微博](#8.20)
*  [抢红包算法设计](#8.21)
*  [设计一个短链系统](#8.22)
*  [降级 限流 熔断实现原理](#8.23)
*  [布隆过滤器的实现原理和使用场景](#8.24)
*  [进程间通信有哪几种方式](#8.25)
*  [进程线程协程区别](#8.26)
*  [502 504 什么原因，如何处理](#8.27)
*  [TCP 粘包如何解决](#8.28)
</details>
...
### 
[<svg class="octicon octicon-link" viewbox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M7.775 3.275a.75.75 0 001.06 1.06l1.25-1.25a2 2 0 112.83 2.83l-2.5 2.5a2 2 0 01-2.83 0 .75.75 0 00-1.06 1.06 3.5 3.5 0 004.95 0l2.5-2.5a3.5 3.5 0 00-4.95-4.95l-1.25 1.25zm-4.69 9.64a2 2 0 010-2.83l2.5-2.5a2 2 0 012.83 0 .75.75 0 001.06-1.06 3.5 3.5 0 00-4.95 0l-2.5 2.5a3.5 3.5 0 004.95 4.95l1.25-1.25a.75.75 0 00-1.06-1.06l-1.25 1.25a2 2 0 01-2.83 0z"></path></svg>](#%E5%88%86%E5%B8%83%E5%BC%8F%E7%AF%87)其他
